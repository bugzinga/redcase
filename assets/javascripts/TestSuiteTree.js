...
Redcase.TestSuiteTree.CheckCallback = function(
	operation,
	node,
	node_parent,
	node_position,
	...
	// Operation can be 'create_node', 'rename_node', 'delete_node', 'move_node'
	// or 'copy_node'.
	var IsOK = true;
...
Redcase.TestSuiteTree.IsDraggable = function(nodes) {
	// Make sure the user can't drag the root node, "default" nodes, the
	// "unsorted" node, and disabled nodes.
...
Redcase.TestSuiteTree.MoveTestCase = function(
	new_node,
	org_node,
	new_instance,
	old_instance
...
Redcase.TestSuiteTree.MoveTestSuite = function(
	new_node,
	org_node,
	new_instance,
	old_instance
	...
	var apiParms = jQuery2.extend(
		{},
		Redcase.methods.testSuite.actions.update(org_node.original.suite_id), {
			params: {
...
Redcase.TestSuiteTree.OnCopy = function(event, object) {
	...
	if (object.old_instance === object.new_instance) {
		...
		case 'case':
			Redcase.TestSuiteTree.MoveTestCase(
				object.node,
				object.original,
				object.new_instance,
				object.old_instance
			...
			break;
		case 'suite':
			Redcase.TestSuiteTree.MoveTestSuite(
				object.node,
				object.original,
				object.new_instance,
				object.old_instance
...
Redcase.TestSuiteTree.contextCopyTo = function(params) {
	var node = Redcase.TestSuiteTree.tree.get_node(params.reference);
	var apiParms = jQuery2.extend(
		{},
		Redcase.methods.testCase.actions.copy(node.original.issue_id), {
			params : {
...
Redcase.TestSuiteTree.deleteCase = function(node) {
	var apiParms = jQuery2.extend(
		...
		Redcase.methods.testCase.actions.update(node.original.issue_id), {
			...
			success: function() {
				var org = jQuery2.extend({}, node.original);
				Redcase.TestSuiteTree.tree.delete_node(node);
				var newId = Redcase.TestSuiteTree.tree.create_node(
					Redcase.TestSuiteTree.tree.get_node('.Obsolete'),
					org
				);
...
Redcase.TestSuiteTree.deleteSuite = function(node) {
	if ((node.parents.length > 1)
		...
		var apiParms = jQuery2.extend(
			{},
			Redcase.methods.testSuite.actions.destroy(node.original.suite_id), {
				success: function () {
					Redcase.TestSuiteTree.tree.delete_node(node);
				},
				errorMessage: (
...
Redcase.TestSuiteTree.deleteItem = function(params) {
	var selected = Redcase.TestSuiteTree.tree.get_selected(true);
				...
				var apiParms = jQuery2.extend(
					...
					Redcase.methods.testSuite.actions.create(), {
						...
						success: function(newNode) {
							Redcase.TestSuiteTree.tree.create_node(
								node,
								newNode
							);
						},
...
Redcase.TestSuiteTree.build = function(params) {
	Redcase.TestSuiteTree.tree = jQuery2('#management_test_suite_tree_id')
		.jstree({
			core: {
				...
				data: {
					type: 'GET',
					url: (
						Redcase.context + Redcase.methods.testSuite.controller
					)
				}
			},
	...
	Redcase.TestSuiteTree.tree.on(
		'copy_node.jstree',
		Redcase.TestSuiteTree.OnCopy
	);
	Redcase.TestSuiteTree.tree = jQuery2.jstree.reference(
		Redcase.TestSuiteTree.tree
	);
};
