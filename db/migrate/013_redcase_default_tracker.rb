...
class RedcaseDefaultTracker < ActiveRecord::Migration
  ...
  def self.up
    ...
    new_status = IssueStatus.create(:name => "New", :is_closed => false) if not new_status
    active_status = IssueStatus.find_by_name('In Progress')
    active_status = IssueStatus.create(:name => "In Progress", :is_closed => false) if not active_status
    obsoleted_status = IssueStatus.find_by_name('Obsoleted')
    obsoleted_status = IssueStatus.create(:name => "Obsoleted", :is_closed => false) if not obsoleted_status
    Role.all.each { |role|
